 ...
 * @author Azael Reyes
 */
public class MecanicoComboBoxModel extends DefaultComboBoxModel<Person>
{    
    ...
    {
        List<Person> ls = Person.listing(db, Person.Type.MECANICO,office,Person.OrderBy.N1AP);  
    ...
    public boolean search(Database db,Office office,int limit,String[] text) throws SQLException
    {
        ...
        List<Person> ls = Person.search(db, Person.Type.MECANICO,office,Person.OrderBy.N1AP,limit,text);  
        //addElement(new Person(-1000));
        for(Person p : ls)
        {
            addElement(p);
