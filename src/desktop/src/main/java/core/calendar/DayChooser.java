 ...
 */
public class DayChooser extends JPanel implements ActionListener,KeyListener,FocusListener 
{
    protected Day[] days;
        ...
        try
        {
            ...
            if(reclicleConextion)
            {
                if(database != null)
                {
                    ...
                    {
                        if(database.getConnection().isValid(50))
                        {
                            return;
                        }
                        else
			...
			if (y == 0) 
                        {
                            ...
                            days[index] = new DecoratorDay();
			} 
                        else 
                        {
                            days[index] = new Day("x") 
                            {
		...
		while (day.before(firstDayInNextMonth)) 
			...
			while (iterator.hasNext()) 
				...
				if (dateEvaluator.isInvalid(day))
                    ...
                    if(marking != null)
                    {
                        days[i + n + 7].setMarking(marking);
                        days[i + n + 7].setDB(database);
                        Calendar cal = Calendar.getInstance();
                        cal.set(tmpCalendar.get(Calendar.YEAR), tmpCalendar.get(Calendar.MONTH), n + 1);
                        days[i + n + 7].setDate(cal.getTime());
                        days[i + n + 7].setPanel(dayPanel);
                    }
                    n++;
	...
	public void setDay(int d) {
		...
		if (selectedDay != null) 
                ...
                //buscar el boton correspondiente para sombrear.
		for (int i = 7; i < 49; i++) 
                {
    ...
    class DecoratorDay extends Day 
    {
