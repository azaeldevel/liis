 ...
 * @author Azael Reyes
 */
public class AssociateImportFolios extends javax.swing.JPanel {
    ...
    private void btImportActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btImportActionPerformed
        ...
        for(String folio : strs)
        {
            ...
            try 
            {
                ...
                ret1 = Invoice.exist(dbserver, office, folio, Operational.Type.SalesInvoice);
                if(ret1.isFail())
                {
                    progress.setProgress(progressStep, "Importando " + folio);
                    invoice = new Invoice(-1);
                    ret2 = invoice.fromCN(dbserver, office, folio, SIIL.servApp.cred.getUser());
                    if(ret2.isFail())
                    {
                        JOptionPane.showMessageDialog(this,
                                ret2.getMessage(),
        ...
        progress.setProgress(0, "Fin de importacion de folio.");
        if(ret2 != null)
        {
            try 
            {
