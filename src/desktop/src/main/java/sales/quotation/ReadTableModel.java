...
public class ReadTableModel extends AbstractTableModel
{
    ...
    public ReadTableModel(List<Quotation> list)
    {
        this.list = list;
    }
    ...
    {
        String sql = "SELECT id FROM " + Quotation.MYSQL_AVATAR_TABLE + " ORDER BY id DESC LIMIT 200";
        ResultSet rs = database.query(sql);    
        list = new ArrayList<>();
        while(rs.next())
        {
            list.add(new Quotation(rs.getInt(1)));            
        }
    ...
    public Quotation getValueAt(int row)
    {
        return list.get(row);
    }
