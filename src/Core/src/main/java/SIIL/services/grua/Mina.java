...
import static SIIL.services.grua.Forklift.MYSQL_AVATAR_TABLE;
 ...
 * @author Azael Reyes
 */
public class Mina extends database.mysql.stock.Titem
{
    public static final String MYSQL_AVATAR_TABLE = "Minas";


    public static boolean isMina(Database database, Titem titem) throws SQLException 
    {
    ...
    public boolean isMina(Database database) throws SQLException 
    {
    ...
    public Return<Integer> insert(Connection connection, String number,String description,Import backward,boolean activeSerie,String serie) throws SQLException
    {
        ...
        {            
            selectNumber(connection, number);
            ret = new Return<>(true, super.getID());
        }
        else
        {
            ret = super.insert(connection, number,description,backward,activeSerie,serie);
        }
